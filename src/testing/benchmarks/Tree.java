...
public class Tree extends UniversalActor  {
	...
	public Tree(UAN __uan, UAL __ual) {
		if (__ual != null && !__ual.getLocation().equals(ServiceFactory.getReception().getLocation())) {
			createRemotely(__uan, __ual, "src.testing.benchmarks.Tree");
		} else {
		...
		public void act(String[] arguments) {
			...
			Token synchToken = new Token("synchToken");
			{
				...
				// token synchToken = standardOutput<-println("Loading actor graph randomly on theater(s): ")
				{
					...
					Message __message = new Message( self, standardOutput, "println", _arguments, token_2_9, synchToken );
					__messages.add( __message );
				}
			}
						...
						if (((i*width)+j+1)<size) {
							{
								...
								// nodes[i]<-addReference(nodes[(i*width)+j+1])
								{
									...
									Message __message = new Message( self, nodes[i], "addReference", _arguments, token_2_0, token_6_0 );
									__messages.add( __message );
								}
